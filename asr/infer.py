    ...
    with open(manifest, "r") as input_file:
        for line in input_file:
            ...
            files.append(os.path.join(
                manifest_path, as_dict["audio_filepath"]))
...
def infer_beam_search_lm(files, asr_model, beam_search_lm):
    hyps = []
    logits = torch.tensor(asr_model.transcribe(
        files, batch_size=20, logprobs=True))
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description='Запуск вывода используя контрольную точку NeMo')
    parser.add_argument(
        'asr_ckpt', help='Путь к контрольной точки NeMo для ASR (.nemo)')
    parser.add_argument('manifest', help='Путь к манифесту где каждая строка это json строка'
                                         ' transcription (.jsonl)')
    parser.add_argument(
        '--lm', help='Путь к KenLM бинарному файлу (.binary)', default=None)
...
if __name__ == "__main__":
    ...
    print("WER:", nemo_asr.metrics.wer.word_error_rate(hyps, texts))

    if args.lm:
        vocab_list = list(asr_model.decoder.vocabulary)
        print(vocab_list[:10])
